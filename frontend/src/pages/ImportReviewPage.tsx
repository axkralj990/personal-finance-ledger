...
import type { ImportBatch, StagedTransaction } from "../api/types";
...
const draftStatuses = new Set(["UPLOADED", "AWAITING_MAPPING", "STAGING", "PARSED", "NEEDS_REVIEW", "READY"]);
...
export default function ImportReviewPage({ embedded = false, readOnly = false, onBatchChanged }: { embedded?: boolean; readOnly?: boolean; onBatchChanged?: (batch: ImportBatch) => void }) {
  const { id = "" } = useParams();
  ...
  const refreshBatch = useEffectEvent((currentBatch: ImportBatch) => {
    onBatchChanged?.(currentBatch);
    if (!embedded) batch.reload();
  });
  ...
  useEffect(() => {
    batchRevisionRef.current = batch.data?.revision ?? 0;
  }, [batch.data?.revision]);
  ...
  useEffect(() => {
    ...
    queueRef.current = new RowSaveQueue({
      save: async (patch) => {
        try {
          const saved = await writeQueueRef.current.run(async () => {
            const result = await api.imports.patchRows(id, batchRevisionRef.current, [patch]);
            const currentBatch = await api.imports.detail(id);
            batchRevisionRef.current = currentBatch.revision;
            refreshBatch(currentBatch);
            return result;
          });
          const row = saved[0];
      ...
      onConflict: async (rowId, unsaved) => {
        try {
          const [currentRows, currentBatch] = await Promise.all([api.imports.rows(id), api.imports.detail(id)]);
          batchRevisionRef.current = currentBatch.revision;
          refreshBatch(currentBatch);
  ...
  }, [id, onBatchChanged, readOnly]);

  const activeFilter = filter ?? (readOnly ? "ALL" : defaultReviewFilter(rows));
  ...
  function saveRow(row: StagedTransaction, changes: StagedChanges) {
    if (readOnly || !isActionableRow(row)) return;
  ...
  async function commit() {
    ...
    setActionError(null);
    try {
      await api.imports.commit(id, batch.data.revision);
  ...
  return (
    <>
      ...
      {!embedded && <ol className="workflow workflow-five" aria-label="Import workflow">
        <li className="workflow-step complete" data-step="1"><strong>Select</strong><span>Original retained</span></li>
        <li className="workflow-step complete" data-step="2"><strong>Inspect</strong><span>Table confirmed</span></li>
        <li className="workflow-step complete" data-step="3"><strong>Map</strong><span>{rows.length} staged rows</span></li>
        <li className="workflow-step active" data-step="4" aria-current="step"><strong>Review</strong><span>{unresolved} actionable signals</span></li>
        <li className="workflow-step" data-step="5"><strong>Commit</strong><span>Atomic ledger write</span></li>
      </ol>}
      ...
      {readOnly && <InlineNotice>Audit rows are read-only. Ignored source rows are retained here but are not ledger transactions.</InlineNotice>}
      ...
      <div className="grid-toolbar">
        ...
        {!readOnly && <div className="grid-toolbar-group">
      ...
      {!readOnly && <p className="notice" aria-live="polite">Bulk actions apply only to {selectedVisibleRows.length} selected visible actionable {selectedVisibleRows.length === 1 ? "row" : "rows"}.</p>}
      ...
      {visibleRows.length ? !isMobile ? (
        <div className="grid-frame">
          <AgGridReact<StagedTransaction>
            ...
            rowSelection={{
              mode: readOnly ? "singleRow" : "multiRow",
              checkboxes: !readOnly,
              headerCheckbox: !readOnly,
              enableClickSelection: false,
              isRowSelectable: ({ data }) => Boolean(!readOnly && data && isActionableRow(data)),
            }}
          ...
          {mobileRows.map((row) => (
            <article className="record-card" key={row.id}>
              <div className="record-card-header"><div>{isActionableRow(row) && <label className="checkbox-field"><input type="checkbox" checked={selectedIds.includes(row.id)} onChange={(event) => setSelectedIds((current) => event.target.checked ? [...new Set([...current, row.id])] : current.filter((item) => item !== row.id))} /> Select row {row.rowNumber}</label>}<h3>{row.description ?? "Description unavailable"}</h3><p>{formatDate(row.date)} / row {row.rowNumber}</p></div><strong className="amount">{formatMoney(row.amountMinor, row.currency)}</strong></div>
               {!readOnly && isActionableRow(row) ? <div className="form-grid">
              ...
              </div> : <InlineNotice>{readOnly ? "This import audit row is read-only." : "Historical and blocked rows are view-only."}</InlineNotice>}
      ...
      <Drawer
        ...
        footer={!readOnly && detail && isActionableRow(detail) && (saveStates[detail.id] === "CONFLICT" || saveStates[detail.id] === "ERROR") ? <button className="button secondary" onClick={() => retryRow(detail)}><RotateCcw aria-hidden="true" /> Retry row save</button> : undefined}
      >
        {detail && (
          <>
            ...
            {!readOnly && detail.errors.length > 0 && isActionableRow(detail) && repair && <section className="detail-section"><h3>Repair invalid row</h3><div className="form-grid">
            ...
            <section className="detail-section"><h3>Prediction</h3><dl className="detail-list"><dt>Category</dt><dd>{categoryName(taxonomy, detail.predictedCategoryId)}</dd><dt>Subcategory</dt><dd>{subcategoryName(taxonomy, detail.predictedSubcategoryId)}</dd><dt>Confidence</dt><dd>{detail.confidence === null ? "No model prediction" : `${Math.round(detail.confidence * 100)}%`}</dd></dl>{!readOnly && hasSuggestedLabels(detail) && <button className="button secondary" onClick={() => acceptSuggestion(detail)}>Accept suggestion</button>}{!readOnly && isActionableRow(detail) && <label className="checkbox-field"><input type="checkbox" checked={detail.rememberCorrection} onChange={(event) => saveRow(detail, { rememberCorrection: event.target.checked })} /> Remember this correction as an exact-match rule</label>}</section>
